...
//  WMGLKitTextureVC.m
//  WMVisualWorld
//  苹果对opengles做了一些封装，开发这可以直接使用GLKit框架做开发，并提供默认着色器GLKBaseEffect，因此GLKBaseEffect不能自定义着色器。
//  Created by wangmm on 2021/8/2.
//

#import "WMGLKitTextureVC.h"

@interface WMGLKitTextureVC ()<GLKViewDelegate>
@property (nonatomic, strong) GLKView *wmGLKView;
@end

@implementation WMGLKitTextureVC
{
    EAGLContext *context;
...
{
    //1.获取纹理图片路径
    NSString *filePath = [[NSBundle mainBundle]pathForResource:@"monika" ofType:@"jpeg"];
    if (!filePath) {
        NSLog(@"there is no image to show,filePath is nil,哈哈");
        return;
    }
    //2.设置纹理参数
...
-(void)setUpVertexData
{
    ...
    GLfloat vertexData[] = {
        ...
        0.5, -0.3, 0.0f,    1.0f, 0.0f, //右下
        0.5, 0.3,  0.0f,    1.0f, 1.0f, //右上
        -0.5, 0.3, 0.0f,    0.0f, 1.0f, //左上
        -0.5, -0.3, 0.0f,   0.0f, 0.0f, //左下
    };
...
-(void)setUpConfig
{
    ...
    GLKView *view =(GLKView *) self.wmGLKView;
...
{
    if (!_wmGLKView) {
        _wmGLKView = [[GLKView alloc] initWithFrame:self.view.bounds];
        _wmGLKView.delegate = self;
    }
    return _wmGLKView;
}
