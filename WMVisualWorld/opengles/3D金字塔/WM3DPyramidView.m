...
//  WM3DPyramidView.m
//  WMVisualWorld
//  使用GLESMath第三方框架做矩阵变换，实际上可以直接使用系统自带的API
...
@implementation WM3DPyramidView
{
    float xDegree;
    float yDegree;
    float zDegree;
...
-(void)render
{
    ...
    NSString* vertFile = [[NSBundle mainBundle] pathForResource:@"shaderv2" ofType:@"glsl"];
    NSString* fragFile = [[NSBundle mainBundle] pathForResource:@"shaderf2" ofType:@"glsl"];
    ...
    //注意：第二参数字符串必须和shaderv2.vsh中的输入变量：position保持一致
    ...
    //10.--------处理顶点颜色值-------
    //(1).glGetAttribLocation,用来获取vertex attribute的入口的.
    //注意：第二参数字符串必须和shaderv2.glsl中的输入变量：positionColor保持一致
...
#pragma mark - XYZrotation
-(void)rotationDegree
{
    //开启定时器
    if (!myTimer) {
        myTimer = [NSTimer scheduledTimerWithTimeInterval:0.05 target:self selector:@selector(rotationDegree) userInfo:nil repeats:YES];
    }
