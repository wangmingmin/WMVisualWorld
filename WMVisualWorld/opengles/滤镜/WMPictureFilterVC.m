...
@property (nonatomic, strong) NSString *vertexShaderName;//自定义顶点着色器
@property (nonatomic, strong) NSString *fragmentShaderName;//自定义片元着色器
...
@implementation WMPictureFilterVC
...
{
    dispatch_queue_t decoderQueue = dispatch_queue_create("wangmm save image", NULL);
    dispatch_async(decoderQueue, ^{
        ...
        WMOpenglesCreateImageTool *tool = [[WMOpenglesCreateImageTool alloc] init];
        UIImage *newImage = [tool createImage:image vertexShader:self.vertexShaderName fragmentShader:self.fragmentShaderName positionSlot:@"Position" textureCoordsSlot:@"TextureCoords" textureSlot:@"Texture"];
        if (newImage) {
            [tool saveImage:newImage andFinised:^(BOOL success) {
...
-(void)setUpFilterChooseView
{
    ...
    //@"正常",@"二分屏",@"九分屏",@"灰度",@"六边形马赛克",@"三角形马赛克",@"圆形浴帘马赛克",@"翻转",@"缩放",@"灵魂出窍",@"抖动",@"闪白",@"毛刺",@"幻影"
    typeof(self)__weak weakSelf = self;
    self.filterScroll.filterBlock = ^(NSInteger index) {
        if (index==0) {
            [weakSelf setupNormalShaderProgram];
        }
        if (index==1) {
            [weakSelf setupSplitScreen_2ShaderProgram];
        }
        if (index==2) {
            [weakSelf setupSplitScreen_9ShaderProgram];
        }
        if (index==3) {
            [weakSelf setupGrayShaderProgram];
        }
        if (index==4) {
            [weakSelf setupHexagonMosaicShaderProgram];
        }
        if (index==5) {
            [weakSelf setupTriangularMosaicShaderProgram];
        }
        if (index==6) {
            [weakSelf setupCircularMosaicShaderProgram];
        }
        if (index==7) {
            [weakSelf setupReversalShaderProgram];
        }
        //动效
        if (index==8) {
            [weakSelf setupScaleShaderProgram];
        }
        if (index==9) {
            [weakSelf setupSoulOutShaderProgram];
        }
        if (index==10) {
            [weakSelf setupShakeShaderProgram];
        }
        if (index==11) {
            [weakSelf setupShineWhiteShaderProgram];
        }
        if (index==12) {
            [weakSelf setupGitchShaderProgram];
        }
        if (index==13) {
            [weakSelf setupVertigoShaderProgram];
        }
        if (index < 7) {
            if (weakSelf.displayLink) {
                [weakSelf.displayLink invalidate];
                weakSelf.displayLink = nil;
            }
            [weakSelf renderScreen];
        }else {
            //动效滤镜，需要时间参数
            [weakSelf startFilerAnimation];
        }
...
{
    if (!_filterScroll) {
        _filterScroll = [[WMChooseFilterScroll alloc] initWithFrame:CGRectMake(0, 100+self.view.frame.size.width+20, self.view.frame.size.width, 80)];
    }
...
// 圆形浴帘马赛克滤镜着色器程序
- (void)setupCircularMosaicShaderProgram {
    [self setupShaderProgramWithName:@"circularMosaic"];
}
...
- (GLuint)programWithShaderName:(NSString *)shaderName {
    self.vertexShaderName = shaderName;
    self.fragmentShaderName = shaderName;
