...
@implementation WMMetalMuchVerRenderer
{
    //渲染的设备(GPU)
...
- (void)loadMetal:(nonnull MTKView *)mtkView
{
    ...
    //从库中加载顶点函数
    id<MTLFunction> vertexFunction = [defaultLibrary newFunctionWithName:@"vertexMuchVerShader"];
    //从库中加载片元函数
    id<MTLFunction> fragmentFunction = [defaultLibrary newFunctionWithName:@"fragmentMuchVerShader"];
    ...
    //5.获取顶点数据
    NSData *vertexData = [WMMetalMuchVerRenderer generateVertexData];
    ...
    _numVertices = vertexData.length / sizeof(WMVertex);
...
{
    //1.正方形 = 三角形+三角形（一个vector_float4有4个字节，一个float占4位，4*4=16）
    const WMVertex quadVertices[] =
    {
        // Pixel 位置, RGBA 颜色
        { { -20,   20 , 0 ,1},    { 1, 0, 0, 1 } },
        { {  20,   20 , 0 ,1},    { 1, 0, 0, 1 } },
    ...
    //顶点个数
    const NSUInteger NUM_VERTICES_PER_QUAD = sizeof(quadVertices) / sizeof(WMVertex);
    //四边形间距
    ...
    //当前四边形
    WMVertex * currentQuad = vertexData.mutableBytes;
    ...
    for(NSUInteger row = 0; row < NUM_ROWS; row++)
    {
        ...
        for(NSUInteger column = 0; column < NUM_COLUMNS; column++)
        {
            ...
            for (NSUInteger vertexInQuad = 0; vertexInQuad < NUM_VERTICES_PER_QUAD; vertexInQuad++)
            {
                ...
                currentQuad[vertexInQuad].position.xy += upperLeftPosition;
            }
