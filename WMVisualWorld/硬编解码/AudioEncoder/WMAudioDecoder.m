...
static OSStatus AudioDecoderConverterComplexInputDataProc(  AudioConverterRef inAudioConverter, UInt32 *ioNumberDataPackets, AudioBufferList *ioData,  AudioStreamPacketDescription **outDataPacketDescription,  void *inUserData) {
    ...
    WMAudioUserData *audioDecoder = (WMAudioUserData *)(inUserData);
...
- (void)decodeAudioAACData:(NSData *)aacData {
    if (!_audioConverter) {
//        [self setupEncoder];//已经在initWithConfig:中调用此方法创建了解码器，可根据自己的需要决定是在这里调用还是在初始化中调用
    }
    if (!_audioConverter) {
        return;//此处return是因为解码器没有创建成功。
    }
    ...
    dispatch_async(_decoderQueue, ^{
        ...
        //输出大小和packet个数
        UInt32 pcmBufferSize = (UInt32)(2048 * weakSelf.config.channelCount);
        ...
        //配置填充函数，获取输出数据
        OSStatus status = AudioConverterFillComplexBuffer(weakSelf.audioConverter, &AudioDecoderConverterComplexInputDataProc, &userData, &pcmDataPacketSize, &outAudioBufferList, &outputPacketDesc);
        if (status != noErr) {
        ...
        if (outAudioBufferList.mBuffers[0].mDataByteSize > 0) {
            ...
            dispatch_async(weakSelf.callbackQueue, ^{
                if ([weakSelf.delegate respondsToSelector:@selector(audioDecodeCallback:)]) {
