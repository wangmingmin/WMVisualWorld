...
@property (nonatomic) BOOL isHeader;//存入文件时用来判断ADTS头

@end

@implementation WMAudioEncoder
...
static OSStatus aacEncodeInputDataProc(AudioConverterRef inAudioConverter, UInt32 *ioNumberDataPackets, AudioBufferList *ioData, AudioStreamPacketDescription **outDataPacketDescription, void *inUserData) {
    ...
    //获取self
    WMAudioEncoder *aacEncoder = (__bridge WMAudioEncoder *)(inUserData);
...
- (instancetype)initWithConfig:(WMAudioConfig*)config {
    ...
    if (self) {
        ...
        _config = config;
        if (config == nil) {
            _config = [[WMAudioConfig alloc] init];
        }
...
- (void)encodeAudioSamepleBuffer: (CMSampleBufferRef)sampleBuffer {
    ...
    dispatch_async(_encoderQueue, ^{
        ...
        //将_pcmBufferSize数据从0开始，set到pcmBuffer中.
        ...
        //PCM数据(不是文件类型时) -> 实时获取，实时编码
        AudioBufferList outAudioBufferList = {0};
        outAudioBufferList.mNumberBuffers = 1;
        outAudioBufferList.mBuffers[0].mNumberChannels = (uint32_t)weakSelf.config.channelCount;
        outAudioBufferList.mBuffers[0].mDataByteSize = (UInt32)weakSelf.pcmBufferSize;
         ...
         */
        status = AudioConverterFillComplexBuffer(weakSelf.audioConverter, aacEncodeInputDataProc, (__bridge void * _Nullable)(self), &outputDataPacketSize, &outAudioBufferList, NULL);
        ...
        if (status == noErr) {
            ...
            NSMutableData *fullData = [NSMutableData dataWithCapacity:rawAAC.length];

            if (self.isWriteToFile) {
                //重要：添加ADTS头，想要获取裸流时，请忽略添加ADTS头；写入文件时，必须添加
                if (self.isHeader == NO) {
                    NSData *adtsHeader = [self adtsDataForPacketLength:rawAAC.length];
                    self.isHeader = YES;
                    fullData = [NSMutableData dataWithCapacity:adtsHeader.length + rawAAC.length];
            ...
            dispatch_async(weakSelf.callbackQueue, ^{
                if ([weakSelf.delegate respondsToSelector:@selector(audioEncodeCallback:)]) {
                    if (self.isWriteToFile) {
                        [weakSelf.delegate audioEncodeCallback:fullData];//写入文件时，回调fullData
                    }else {
                        [weakSelf.delegate audioEncodeCallback:rawAAC];//实时裸流，直接回调rawAAC
                    }
                }
            });
        } else {
...
- (void)setupEncoderWithSampleBuffer: (CMSampleBufferRef)sampleBuffer {
    ...
    //获取输入参数pcm，此时并不知道原始PCM信息，只能从sampleBuffer中获取
    ...
    outputAudioDes.mChannelsPerFrame = (uint32_t)_config.channelCount; //输出声道数1;双声道就设置2
