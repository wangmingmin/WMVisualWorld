...
@implementation WMVideoEncoder{
    __block long frameID;   //帧的递增序标识
...
void VideoEncodeCallback(void * CM_NULLABLE outputCallbackRefCon, void * CM_NULLABLE sourceFrameRefCon,OSStatus status, VTEncodeInfoFlags infoFlags,  CMSampleBufferRef sampleBuffer ) {
    ...
    WMVideoEncoder *encoder = (__bridge WMVideoEncoder *)(outputCallbackRefCon);
    ...
    while (offet < totalLength - lengthInfoSize) {
        ...
        //获取一单元长度的nalu 数据长度
        //dataPoint + offet: 首地址dataPoint+步长offet，指针偏移
        //memcpy是具备字符串拷贝功能的函数，这是一个内存拷贝函数，它的 函数原型为memcpy(char dst, const char src, unsigned int len);将长度为len 的一段内存，从src拷贝到dst中去，这个函数的长度可控。但是会有内存 叠加的问题
