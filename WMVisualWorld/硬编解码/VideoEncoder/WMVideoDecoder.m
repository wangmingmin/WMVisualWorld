...
@implementation WMVideoDecoder{
    uint8_t *_sps;
    NSUInteger _spsSize;
    uint8_t *_pps;
    NSUInteger _ppsSize;
    CMVideoFormatDescriptionRef _decodeDesc;//视频输出格式，(重要)需要根据它才能完整的进行解码
}
...
void videoDecompressionOutputCallback(void * CM_NULLABLE decompressionOutputRefCon,
    ...
    //重要：解码后的数据sourceFrameRefCon -> CVPixelBufferRef
    CVPixelBufferRef *outputPixelBuffer = (CVPixelBufferRef *)sourceFrameRefCon;
    *outputPixelBuffer = CVPixelBufferRetain(imageBuffer);
    
    //获取self
    WMVideoDecoder *decoder = (__bridge WMVideoDecoder *)(decompressionOutputRefCon);
    
    //调用回调队列
    dispatch_async(decoder.callbackQueue, ^{
        
        //将解码后的数据给decoder代理.viewController
        //可以传*outputPixelBuffer也可以传imageBuffer
//        [decoder.delegate videoDecodeCallback:*outputPixelBuffer];
...
- (BOOL)initDecoder {
    ...
    if (_decodeSesion) return true;//解码器只用创建一次，不需要每次都创建
    ...
    * kCVPixelBufferWidthKey/kCVPixelBufferHeightKey: 视频源的分辨率 width*height
     * kCVPixelBufferOpenGLCompatibilityKey : 它允许在 OpenGL 的上下文中直接绘制解码后的图像，而不是从总线和 CPU 之间复制数据。这有时候被称为零拷贝通道，因为在绘制过程中没有解码的图像被拷贝.在解码过程中减少延时
...
- (CVPixelBufferRef)decode:(uint8_t *)frame withSize:(uint32_t)frameSize {
    /*
     步骤：
     CVPixelBufferRef 解码后的数据/编码前源视频数据
     CMBlockBufferRef 编码后的数据
     需要将 frame包装成 CMBlockBufferRef， 再将CMBlockBufferRef 包装成CMSampleBufferRef，
     因为解码函数需要CMSampleBufferRef数据类型才能解码
     */
    ...
    //这里返回CVPixelBufferRef，但是，最好在videoDecompressionOutputCallback回调方法里返回解码数据
    return outputPixelBuffer;
}
...
- (void)decodeNaluData:(uint8_t *)frame size:(uint32_t)size {
    ...
    /*
     ...
     sps/pps数据:则将sps/pps数据赋值到_sps/_pps中.缓存起来,等待其它方法使用(比如初始化解码器时候)。
     */
    switch (type) {
        case 0x05: //关键帧
            if ([self initDecoder]) {
...
- (void)decodeNaluData:(NSData *)frame {
    ...
    dispatch_async(_decodeQueue, ^{
        //获取二进制数据frame， 将数据拆解，NSData->二进制数据bytes
